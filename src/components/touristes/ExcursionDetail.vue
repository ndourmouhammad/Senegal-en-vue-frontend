<template>
    <div>
      ...
      <div class="container-fluid mt-1">
        <h1 class="titre">Les informations de l'excursion</h1>
        <!-- Carte de l'événement -->
        <div class="card mb-3 card-no-border mt-4" style="border-radius: 30px">
          <div class="banniere" v-if="excursionDetails">
            <video
              v-if="isVideo(excursionDetails?.contenu)"
              :src="getMediaUrl(excursionDetails?.contenu)"
              controls
            ></video>
            <img
              v-else
              :src="getMediaUrl(excursionDetails?.contenu)"
              :alt="excursionDetails?.libelle"
            />
          </div>
          <div v-else>
        ...
        <div class="flex">
          <div class="sites-detail">
            <div class="body_detail mt-4">
              <div class="card-body">
                <h1>{{ excursionDetails?.libelle }}</h1>
                ...
                <div class="info-item">
                  <img
                    ...
                    class="me-2"
                  />
                  {{ excursionDetails?.nombre_participants }} participants
                </div>
                ...
                <div class="info-item">
                  <img
                    ...
                    class="me-2"
                  />
                  {{ excursionDetails?.date_debut }} -
                  {{ excursionDetails?.date_fin }}
                </div>
                ...
                <div class="info-item">
                  <img
                    ...
                    class="me-2"
                  />
                  {{ excursionDetails?.tarif_entree }} FCFA
                </div>
              </div>
            </div>
            <button class="btn btn-primary mb-5" @click="reserver">
              Reserver
            </button>
  ...
  // State variables
  const excursionDetails = ref(null);
  const siteActivities = ref([]); // Initialize as an empty array
  const guideInfo = ref(null);
  const regionInfo = ref(null);
  const reservationMessage = ref("");

  const fetchExcursionData = async (excursionId) => {
    try {
      // Fetch site details
      const excursion = await excursionService.getExcursion(excursionId);
      console.log("Site Details:", excursion);
      excursionDetails.value = excursion.data;

      // Fetch site activities
    
    const activities = await excursionService.getExcursionActivites(excursionId);
    siteActivities.value = activities.data;
    console.log("Site Activities:", activities);
  ...
  const validActivities = computed(() => {
    return siteActivities.value.filter((activity) => activity != null); // Ensure siteActivities is an array
  });
  ...
  onMounted(() => {
    fetchExcursionData(excursionId);
  });
</script>
