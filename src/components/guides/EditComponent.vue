<template>
    ...
    <div v-if="site.contenu">
        <template v-if="isVideo(site.contenu)">
            <video controls :src="getMediaUrl(site.contenu)" class="mt-2" style="max-width: 50px; height: 50px"></video>
        </template>
        <template v-else>
            <img :src="getMediaUrl(site.contenu)" alt="Image existante" class="mt-2" style="max-width: 50px; height: 50px" />
        </template>
    </div>
</div>
